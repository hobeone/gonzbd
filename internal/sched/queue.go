...
// internal/job/job.go's surrenderLocked comment uses) so it does not inflate
// that count. Prior spec §7.1's order is Queue.mu before Job.mu, so nothing
// here may call into a *job.Job method while holding mu in a way that would
...
// SetCaps dynamically updates the lease and slot pool capacities.
func (q *Queue) SetCaps(leaseCap, slotCap int) {
	q.mu.Lock()
	defer q.mu.Unlock()
	q.leases.setCapacity(leaseCap)
	q.slots.setCapacity(slotCap)
}

// LeaseCap returns the current capacity of the lease pool.
func (q *Queue) LeaseCap() int {
	q.mu.Lock()
	defer q.mu.Unlock()
	return q.leases.capacity
}

// SlotCap returns the current capacity of the slot pool.
func (q *Queue) SlotCap() int {
	q.mu.Lock()
	defer q.mu.Unlock()
	return q.slots.capacity
}
