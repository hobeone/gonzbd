...
func TestSnapshotJobByName_Audit(t *testing.T) {
	t.Parallel()

	t.Run("finds job by name", func(t *testing.T) {
		t.Parallel()
		q := New()
		j := makeJob(t, "named-job", constants.NormalPriority)
		_ = q.Add(j)

		snap := q.SnapshotJobByName("named-job")
		if snap == nil {
			t.Fatal("SnapshotJobByName returned nil for existing name")
		}
		if snap.ID != j.ID {
			t.Errorf("ID = %q, want %q", snap.ID, j.ID)
		}
		if snap.Name != "named-job" {
			t.Errorf("Name = %q, want %q", snap.Name, "named-job")
		}
	})

	t.Run("returns nil for nonexistent name", func(t *testing.T) {
		t.Parallel()
		q := New()
		_ = q.Add(makeJob(t, "other", constants.NormalPriority))
		if snap := q.SnapshotJobByName("nonexistent"); snap != nil {
			t.Errorf("expected nil, got %+v", snap)
		}
	})
	...
	t.Run("case-sensitive lookup", func(t *testing.T) {
		t.Parallel()
		q := New()
		_ = q.Add(makeJob(t, "CaseSensitive", constants.NormalPriority))
		if snap := q.SnapshotJobByName("casesensitive"); snap != nil {
			t.Error("SnapshotJobByName should be case-sensitive")
		}
		if snap := q.SnapshotJobByName("CaseSensitive"); snap == nil {
			t.Error("SnapshotJobByName should find exact case match")
		}
	})

	t.Run("returns deep copy (mutation isolation)", func(t *testing.T) {
		t.Parallel()
		q := New()
		j := makeJob(t, "snap-name-copy", constants.NormalPriority)
		_ = q.Add(j)

		snap := q.SnapshotJobByName("snap-name-copy")
		if snap == nil {
			t.Fatal("SnapshotJobByName returned nil")
		}
		snap.Name = "mutated"
		got, _ := q.Get(j.ID)
		if got.Name == "mutated" {
			t.Error("mutation of snapshot name leaked to original")
		}
	})

	t.Run("finds first matching name when duplicates exist", func(t *testing.T) {
		t.Parallel()
		q := New()
		// Add two jobs; the second will be renamed to "dup.1" by queue.Add.
		j1 := makeJob(t, "dup", constants.NormalPriority)
		j2 := makeJob(t, "dup", constants.NormalPriority)
		_ = q.Add(j1)
		_ = q.Add(j2)

		snap := q.SnapshotJobByName("dup")
		if snap == nil {
			t.Fatal("SnapshotJobByName returned nil for 'dup'")
		}
		if snap.ID != j1.ID {
			t.Errorf("expected first job's ID %q, got %q", j1.ID, snap.ID)
		}
		// Second job was renamed.
		snap2 := q.SnapshotJobByName("dup.1")
		if snap2 == nil {
			t.Fatal("SnapshotJobByName returned nil for 'dup.1'")
		}
		if snap2.ID != j2.ID {
			t.Errorf("expected second job's ID %q, got %q", j2.ID, snap2.ID)
		}
	})
}
