...
// ---------------------------------------------------------------------------
//
// newQueueJob and makeJob now live in testharness_test.go, shared across
...
func (s *recordStage) Run(ctx context.Context, job *Job) error {
	...
	s.mu.Lock()
	s.calls = append(s.calls, s.name+"/"+job.Job.Name())
	s.mu.Unlock()
	if s.runFn != nil {
...
func TestCancelQueuedJob(t *testing.T) {
	...
	found := false
	for _, j := range hist {
		if j.Job.ID() == "second" && len(j.StageLog) > 0 {
			found = true
		}
	}
	if found {
...
func TestOnJobDoneFiredOnce(t *testing.T) {
	...
	p := startProcessor(t, Options{
		...
		OnJobDone: func(j *Job) {
			mu.Lock()
			firings[j.Job.ID()]++
			logs[j.Job.ID()] = append([]StageLogEntry{}, j.StageLog...)
			mu.Unlock()
			wg.Done()
		},
	})
...
func TestPPQueueOrdering(t *testing.T) {
	...
	for _, want := range names {
		...
		if job.Job.Name() != want {
			t.Errorf("got job %q, want %q", job.Job.Name(), want)
		}
	}
}
...
func TestPPEnforcement_PP0SkipsRepairAndUnpack(t *testing.T) {
	...
	job := &Job{Job: newQueueJob(t, "pp0", 0)}
	p.Process(job)
	wg.Wait()
...
func TestPPEnforcement_PP1SkipsUnpack(t *testing.T) {
	...
	job := &Job{Job: newQueueJob(t, "pp1", 1)}
	p.Process(job)
	wg.Wait()
...
func TestPPEnforcement_PP2RunsRepairAndUnpack(t *testing.T) {
	...
	job := &Job{Job: newQueueJob(t, "pp2", 2)}
	p.Process(job)
	wg.Wait()
...
func TestPPEnforcement_PP3RunsAll(t *testing.T) {
	...
	job := &Job{Job: newQueueJob(t, "pp3", 3)}
	p.Process(job)
	wg.Wait()
...
func TestPPEnforcement_PP0_NonGatedStagesAlwaysRun(t *testing.T) {
	...
	job := &Job{Job: newQueueJob(t, "pp0-full", 0)}
	p.Process(job)
	wg.Wait()
...
func TestShouldSkipForPP(t *testing.T) {
	...
	for _, tt := range tests {
		got := shouldSkipForPP(tt.stage, job.PolicyFromPP(tt.pp))
		if got != tt.want {
			t.Errorf("shouldSkipForPP(%q, PolicyFromPP(%d)) = %v, want %v", tt.stage, tt.pp, got, tt.want)
		}
	}
}
...
func TestQuickCheckPassedSkipsRepair(t *testing.T) {
	...
	p := startProcessor(t, Options{
		...
		OnJobDone: func(j *Job) {
			doneMu.Lock()
			doneJobs = append(doneJobs, j.Job.ID())
			doneMu.Unlock()
		},
	})
...
func TestPostProc_HelperMethods(t *testing.T) {
	...
	t.Run("buildPreambleLog", func(t *testing.T) {
		now := time.Now()
		qjob := newNamedJob(t, "job1", "TestJob", 0)
		setDownloadStamps(t, qjob, now, now.Add(5*time.Second))
		job := &Job{
			Job:         qjob,
			Meta:        JobMeta{Filename: "job1.nzb"},
	...
	t.Run("runStage", func(t *testing.T) {
		...
		// 2. Normal success run. Policy is bound to job.Job at construction
