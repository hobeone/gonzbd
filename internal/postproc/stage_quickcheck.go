...
func (q *QuickCheckStage) Run(ctx context.Context, job *Job) error {
	...
	log = log.With("component", "quickcheck", "job", job.Job.ID())
...
func (q *QuickCheckStage) assess(job *Job, sets []par2.Set, log *slog.Logger) (par2.Assessment, error) {
	...
	if job.Job != nil && job.Job.NumFiles() > 0 {
		if m, err := job.Job.Manifest(); err == nil {
			p := job.Job.Progress()
...
func (q *QuickCheckStage) recordVerdict(ctx context.Context, log *slog.Logger, job *Job, sets []par2.Set, a par2.Assessment) error {
	...
	// checked (#294).
	//
	// job.Job.Manifest() wraps job.ErrNotResident with %w rather than
	// returning a bare error, which is what keeps this distinguishable from
	// any other manifest failure a caller further up might want to test for
	// with errors.Is — the same distinction #294 needed, carried onto the new
	// door. Unlike internal/queue's Manifest() (which could also fail from a
	// disk read, since a queue.Job's manifest tier could be evicted-then-
	// reloaded on demand), job.Job.Manifest() has exactly one failure mode:
	// ErrNotResident (internal/job/content.go). There is no second "manifest
	// exists but is unreadable" case left to conflate with eviction at this
	// layer — a manifest that fails to parse or load never reaches a resident
	// job.Job in the first place.
	if _, mErr := job.Job.Manifest(); mErr != nil {
