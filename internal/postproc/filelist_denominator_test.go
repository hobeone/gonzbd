...
func TestBuildDownloadFileList_FailurePercentSharesHistoryDenominator(t *testing.T) {
	...
	// The oracle rules the volume unnecessary: FetchNever, replacing
	// DiscardDeferredPar2 (see testharness_test.go's setFileFetchPolicy doc
	// comment for why this is a direct poke rather than a real door call).
	setFileFetchPolicy(t, qjob, 1, job.FetchNever)
	m, err := qjob.Manifest()
	if err != nil {
