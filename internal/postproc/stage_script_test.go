...
// SAB_BYTES because ScriptInput.Bytes read job.Job.TotalBytes(), which no
...
//
// internal/queue.Queue.DiscardDeferredPar2 has no internal/job equivalent
// yet (on-demand par2's discard path is unported -- see
// testharness_test.go's pokeProgressField doc comment), so the discard is
// simulated directly via setFileFetchPolicy rather than driven through a
// real discard call.
func TestScriptStage_BytesExcludesDiscardedPar2(t *testing.T) {
	...
	qjob := newNamedJob(t, "bytes-test", "bytes-test", 0)
	m := buildManifest(t, []testFile{
		{Subject: `"movie.mkv" yEnc`, Bytes: 1000, Articles: []testArticle{{ID: "c@x", Bytes: 1000, Number: 1}}},
		{Subject: `"movie.par2" yEnc`, Bytes: 50, Articles: []testArticle{{ID: "i@x", Bytes: 50, Number: 1}}},
		{Subject: `"movie.vol000+01.par2" yEnc`, Bytes: 500, IsPar2Recovery: true, Articles: []testArticle{{ID: "v@x", Bytes: 500, Number: 1}}},
	})
	if err := qjob.AttachContent(m); err != nil {
		t.Fatalf("AttachContent: %v", err)
	}
	// Discarded: FetchNever, matching what DiscardDeferredPar2 would leave
	// behind for a recovery volume the CRC oracle proved unnecessary.
	setFileFetchPolicy(t, qjob, 2, job.FetchNever)
	...
	job := &Job{Job: qjob, DownloadDir: t.TempDir()}
