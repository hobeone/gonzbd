...
func TestStop_PersistTimeoutIsolatedFromStopCtx(t *testing.T) {
	...
	// Add wrote the job's row; move the job so Stop has a change to persist.
...
func TestStop_PersistErrorAggregatedIntoStopErr(t *testing.T) {
	...
	// The store fails only after Add has written the row, and the job moves so
	// Stop has a change to persist.
	st.mu.Lock()
