...
// deliberately separate from `jobs`, which belongs to internal/queue until
// B2.4 deletes that package — see the table's own comment block in
// internal/history/migrations/001_initial.sql for the argument.
//
// Nothing in production constructs a Store yet. B2.4 repoints the application
// onto internal/dispatch and is where that happens.
package store
