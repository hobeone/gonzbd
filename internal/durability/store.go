...
// Store owns a job's per-job rows in the history database: durable_runs,
// job_files and failed_articles. This file holds the durable_runs half;
// progress.go holds the other two tables.
//
// Run content has one writer, and it is not a method anyone outside this
// package can call: commit is unexported (see runStore). What an exported
// method may do to durable_runs is pinned by
// TestStore_NoExportedMethodWritesRunContent.
//
// For durable_runs, commit
// REPLACEs rather than appending, which is what retired R1's immutability:
...
// merge dropped.
func (s *Store) commitFile(ctx context.Context, tx *sql.Tx, jobID string, fileIdx int32, arts []DurableArticle) ([]Collision, error) {
...
// TestStore_RedeliveredAdjacentToNewIsNotAFalseOverlap exists to
// prevent.
//
...
func (s *Store) queryBracketing(ctx context.Context, tx *sql.Tx, jobID string, fileIdx int32, maxEnd int64) ([]Run, error) {
...
func (s *Store) deleteRows(ctx context.Context, tx *sql.Tx, jobID string, fileIdx int32, rows []Run) error {
...
func (s *Store) insertRuns(ctx context.Context, tx *sql.Tx, jobID string, runs []Run) error {
...
// A row that fails to scan returns nothing; a fault partway through iteration
// returns the runs read before it, with an error wrapping ErrIncomplete.
func (s *Store) ForJob(ctx context.Context, jobID string) ([]Run, error) {
	...
	if err := rows.Err(); err != nil {
		return out, fmt.Errorf("durability: iterate runs job=%s: %w: %w", jobID, ErrIncomplete, err)
	}
	return out, nil
}
