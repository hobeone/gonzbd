...
func TestHandleFileComplete_RecordsACompletionItCouldNotDeliver(t *testing.T) {
	...
	// Pausing and evicting the manifest ensures MarkFileComplete answers
	// ErrNotResident — the interval the defect lives in.
	if err := application.dispatcher.PauseJob(j.ID()); err != nil {
		t.Fatalf("PauseJob: %v", err)
	}
	j.Evict()

	err := application.completeFinalizedFile(t.Context(), FileComplete{JobID: j.ID(), FileIdx: 0})
	...
	application.noteUndeliveredCompletion(j.ID(), 0)

	files := application.recoveryFiles(j.ID())
	st, ok := files[0]
	if !ok {
...
func TestCheckpointJob_RecordsAJobWhoseAckCouldNotLand(t *testing.T) {
	...
	application.noteNeedsSeed(j.ID())

	if !application.needsSeed(j.ID()) {
	...
	if files := application.recoveryFiles(j.ID()); len(files) != 0 {
