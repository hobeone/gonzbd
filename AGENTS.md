...
| `check_doc_citations` | A cited file path that names nothing in the tree, or a `Test...` name cited as a guard that is declared nowhere. Reads Markdown too, and claims with no command behind them. | Correct the reference, or mark it deliberate — `//doccite:ok <token> — <why>` in Go, `<!-- doccite:ok <token> — <why> -->` in Markdown. |
| `check_test_doubles` | Test doubles (`Fake`, `Mock`, `Stub`, `Nop`, `*ForTesting`) or test-named files leaking into production builds without test build tags | Add a recognized test build tag, move to a `*_test.go` file, or add `//testdouble:allow <reason>` on the declaration. |
