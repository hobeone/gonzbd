...
- **This is a convention, not an enforced gate.** There is no GitHub branch protection (private repo; protection is a Pro/public-only feature), so nothing on the server will reject a direct push to `main`. It holds because we follow it. Do not read "the push succeeded" as "the push was allowed."
