...
<Dialog.Root bind:open>
	<Dialog.Portal>
		...
		<Dialog.Content class="fixed left-1/2 top-1/2 z-50 flex h-[85vh] w-full max-w-4xl -translate-x-1/2 -translate-y-1/2 overflow-hidden rounded-lg border border-border bg-card shadow-lg text-foreground">
			<!-- Sidebar -->
			<aside class="w-64 shrink-0 border-r border-border bg-muted/40 p-4">
				...
				<nav class="mt-6 space-y-1">
					{#each sections as section (section.id)}
						<button
							...
							{activeSection === section.id ? 'bg-primary/20 text-primary' : 'text-muted-foreground hover:bg-muted'}"
						>
							{section.label}
						</button>
					{/each}
			...
			<div class="flex flex-1 flex-col overflow-hidden">
				<div class="flex-1 overflow-y-auto p-8">
					{#if error}
						<div class="mb-6 flex items-center justify-between rounded-lg border border-destructive bg-destructive/10 p-4 text-sm text-destructive">
				...
				<!-- Footer -->
				<footer class="flex items-center justify-between border-t border-border bg-muted/40 px-8 py-4">
