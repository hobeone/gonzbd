...
<section class="space-y-6">
	...
	<div>
		...
		<div class="divide-y divide-gray-100 dark:divide-gray-800">
			<ConfigSwitch section="postproc" keyword="enable_unrar" label="Enable RAR extraction" value={configData.postproc.enable_unrar} requiresRestart={true} onupdate={onFieldUpdate} />
			<ConfigSwitch section="postproc" keyword="use_go_rar" label="Use built-in RAR extractor" value={configData.postproc.use_go_rar} description="Use the pure-Go RAR extractor instead of the external unrar binary. No external tools required. Disable to fall back to the subprocess unrar command." requiresRestart={true} onupdate={onFieldUpdate} />
			<ConfigSwitch section="postproc" keyword="go_rar_fallback" label="Fallback to unrar on failure" value={configData.postproc.go_rar_fallback} description="If the built-in RAR extractor fails, automatically retry with the external unrar binary. Disable to treat go_unrar failures as final (no subprocess retry)." requiresRestart={true} onupdate={onFieldUpdate} />
			<ConfigSwitch section="postproc" keyword="enable_7zip" label="Enable 7-Zip extraction" value={configData.postproc.enable_7zip} requiresRestart={true} onupdate={onFieldUpdate} />
			<ConfigSwitch section="postproc" keyword="use_go_7z" label="Use built-in 7-Zip extractor" value={configData.postproc.use_go_7z} description="Use the pure-Go 7-Zip extractor instead of the external 7z binary. No external tools required. Disable to fall back to the subprocess 7z command." requiresRestart={true} onupdate={onFieldUpdate} />
			...
			<ConfigSwitch section="postproc" keyword="enable_filejoin" label="Enable file joining" value={configData.postproc.enable_filejoin} description="Join split files (.001, .002, ...) into a single file." requiresRestart={true} onupdate={onFieldUpdate} />
			<ConfigSwitch section="postproc" keyword="enable_tsjoin" label="Enable TS joining" value={configData.postproc.enable_tsjoin} description="Join split transport stream files (.ts.001, .ts.002, ...)." requiresRestart={true} onupdate={onFieldUpdate} />
			<ConfigSwitch section="postproc" keyword="enable_recursive" label="Recursive unpacking" value={configData.postproc.enable_recursive} description="Re-scan for archives inside extracted files, up to 3 levels deep." requiresRestart={true} onupdate={onFieldUpdate} />
			<ConfigSwitch section="postproc" keyword="direct_unpack" label="Direct Unpack" value={configData.postproc.direct_unpack} description="Extract RAR volumes while still downloading, overlapping I/O for faster completion. Requires 'Enable RAR extraction'." requiresRestart={true} onupdate={onFieldUpdate} />
	...
	<div>
		...
		<div class="divide-y divide-gray-100 dark:divide-gray-800">
			<ConfigSwitch section="postproc" keyword="use_go_par2" label="Use built-in par2 verifier" value={configData.postproc.use_go_par2} description="Use the native par2 engine for verification and repair instead of the external par2 binary. No external tools required. Disable to fall back to the subprocess par2 command." requiresRestart={true} onupdate={onFieldUpdate} />
	...
	<div>
		...
		<div class="divide-y divide-gray-100 dark:divide-gray-800">
			...
			<ConfigInput section="postproc" keyword="par2_command" label="par2 path" placeholder="auto-detect from PATH" value={configData.postproc.par2_command} requiresRestart={true} onupdate={onFieldUpdate} />
			<ConfigInput section="postproc" keyword="unrar_command" label="UnRAR path" placeholder="auto-detect from PATH" value={configData.postproc.unrar_command} requiresRestart={true} onupdate={onFieldUpdate} />
			<ConfigInput section="postproc" keyword="sevenz_command" label="7-Zip path" placeholder="auto-detect from PATH (7zz or 7z)" value={configData.postproc.sevenz_command} requiresRestart={true} onupdate={onFieldUpdate} />
			<ConfigInput section="postproc" keyword="password_file" label="Password file" placeholder="path to passwords.txt" value={configData.postproc.password_file} description="Text file with one password per line, tried during encrypted archive extraction." requiresRestart={true} onupdate={onFieldUpdate} />
		</div>
	</div>
	...
	<div>
		...
		<div class="divide-y divide-gray-100 dark:divide-gray-800">
			...
			<ConfigInput section="postproc" keyword="nice" label="nice arguments" placeholder="e.g. -n 15 (empty = disabled)" value={configData.postproc.nice} description="Prepended to all external tool commands to lower CPU priority." requiresRestart={true} onupdate={onFieldUpdate} />
			<ConfigInput section="postproc" keyword="ionice" label="ionice arguments" placeholder="e.g. -c2 -n4 (empty = disabled)" value={configData.postproc.ionice} description="Prepended to all external tool commands to lower I/O priority." requiresRestart={true} onupdate={onFieldUpdate} />
			<ConfigInput section="postproc" keyword="extra_unrar_params" label="Extra UnRAR parameters" placeholder="e.g. -ri5:100" value={configData.postproc.extra_unrar_params} description="Additional flags for unrar. Only -mlp, -om*, and -ri* are allowed." requiresRestart={true} onupdate={onFieldUpdate} />
			<ConfigInput section="postproc" keyword="extra_par2_params" label="Extra par2 parameters" placeholder="e.g. -m512" value={configData.postproc.extra_par2_params} description="Additional flags for par2 repair invocations." requiresRestart={true} onupdate={onFieldUpdate} />
		</div>
	</div>
</section>
