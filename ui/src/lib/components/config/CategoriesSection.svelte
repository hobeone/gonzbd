				...
				<tbody class="divide-y">
					{#each configData.categories as cat (cat.name)}
						<tr>
							...
							<td class="px-4 py-3 text-muted-foreground">{cat.dir || '(default)'}</td>
							<td class="px-4 py-3">
							...
							<td class="px-4 py-3 text-right">
								<div class="flex justify-end gap-1">
									...
									<Button variant="ghost" size="xs" class="text-destructive" disabled={cat.name === '*' || cat.name === 'Default'} onclick={() => onDeleteCategory(cat.name)}>Delete</Button>
								</div>
							</td>
						</tr>
					{/each}
