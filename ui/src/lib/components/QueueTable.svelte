...
<Dialog.Root bind:open={showDeleteConfirm}>
	<Dialog.Portal>
		...
		<Dialog.Content
			class="fixed left-1/2 top-1/2 z-50 w-full max-w-md -translate-x-1/2 -translate-y-1/2 rounded-lg border border-border bg-card p-6 shadow-lg outline-none text-foreground"
		>
			<div class="mb-4">
				<Dialog.Title class="text-lg font-bold">Delete Job</Dialog.Title>
				<Dialog.Description class="mt-2 text-sm text-muted-foreground">
					Are you sure you want to delete <span class="inline-block max-w-[200px] sm:max-w-xs align-bottom font-semibold text-foreground truncate" title={deleteTarget?.name || deleteTarget?.filename}
			...
			<div class="py-4 text-foreground">
				<label class="flex cursor-pointer items-center gap-2 text-sm">
					<input
						...
						class="size-4 rounded border-input bg-transparent text-foreground focus:ring-ring"
					/>
