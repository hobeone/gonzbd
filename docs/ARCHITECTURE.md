...
- **Single Progress Writer**: `Checkpointer` (`internal/checkpoint`) is the only thing that *updates* `job_files` or *inserts* into `failed_articles` in SQLite, batching and coalescing from job snapshots. It is not the only writer of those tables: ingestion inserts the initial `job_files` rows, and rows are deleted by two job-cleanup paths in `internal/app` plus the history purge in `internal/history`, which drops retained durability rows when a history entry goes. What the checkpointer owns is progress, not row lifetime — `docs/durability-contract.md` § One record carries the deleter enumeration and the command behind it.
