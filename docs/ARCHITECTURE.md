...
- **Clean verdict**: `DiscardDeferredPar2` moves every still-`FetchIfNeeded` volume to `FetchNever`, and only `outcomeClean` reaches it. No *verdict* promotes it back within a run — `Job.undeferRecovery` skips anything that is not `FetchIfNeeded`. Hydration is the exception, and it is not a verdict: `appResidency.Hydrate` restores the policy from `job_files`, so an eviction re-hydrates whatever the row holds. The row lags memory until something marks the job dirty and a flush follows, and neither `DiscardDeferredPar2` nor `UndeferRecoveryVolumes` marks it — a flush alone writes only jobs already marked, so a verdict can sit unpersisted indefinitely once the job has stopped downloading. A retry does not inherit the previous attempt's policy at all — it rebuilds the job from scratch through `BuildIngestJob`, whose own classification of a recovery volume sets it back to `FetchIfNeeded`, so a retry re-derives the verdict by re-verifying instead of re-downloading volumes the oracle already ruled out (#329).
