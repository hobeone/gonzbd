...
- **Manifest/Progress split**: a `Job` holds an immutable `Manifest` (parsed-once article/file structure — subjects, byte counts, the flat global article arrays) and a mutable `JobProgress` (per-article done/failed/emitted state, per-file assembly bookkeeping, job-level counters). `Manifest` is safe to share by reference across every `Snapshot`/`SnapshotJob` clone since nothing mutates it after construction; `JobProgress` is deep-copied per clone. External packages reach both only through `Job.Manifest()`/`Job.Progress()` accessor methods — neither type exposes a mutating exported method, so handing out the pointers is safe.
...
- **ActiveSet & Promotion Loop**: `ActiveSet` manages in-memory resident `(Manifest, JobProgress)` pairs exclusively for active/processing jobs. Pending jobs are stored in SQLite (`queue.db`) with zero resident manifests in RAM. The promotion loop (`Pending -> Active`) is single-caller bounded by `MaxActiveJobs` (default 4).
- **Residency Invariant & RFC Decision (#267)**: A job present in `Queue.byID` is not guaranteed to have an in-memory `manifest` or `progress` (jobs are de-hydrated when `StatusQueued` or `StatusPaused` if an SQLite store is configured). Consequently, internal methods that dereference a job's `manifest` or `progress` must route through `q.residentJob(id)` (returning `ErrJobNotResident` for non-resident jobs) or explicitly call `q.hydrateJobLocked(ctx, job)`. Public accessors (`Job.Manifest()` and `Job.Progress()`) are thread-safe via an internal mutex; methods on `*Manifest` do not include zero-value nil guards (#265), ensuring dereferencing an unhydrated manifest fails fast at the caller site rather than propagating silent zero-value corruption. In RFC #267, we evaluated replacing this checked-pointer convention with a compile-time type hierarchy (`ResidentJob` vs `QueuedJob`), but rejected type splitting: it requires duplicating metadata fields, introduces allocation and copying overhead on every promotion/eviction cycle, and complicates atomic queue snapshots. Checked pointers with `residentJob(id)` enforcement provide structural safety without runtime allocation overhead.
- **Persistence**: Active queue state and job metadata are persisted in SQLite (`queue.db`). Immutable job manifests are stored on disk under `<AdminDir>/queue/manifests/<id>.json.gz`.
