...
- **Residency Invariant & RFC Decision (#267)**: A job present in `Queue.byID` is not guaranteed to have an in-memory `manifest` or `progress` (jobs are de-hydrated when `StatusQueued` or `StatusPaused` if an SQLite store is configured). Consequently, internal methods that dereference a job's `manifest` or `progress` must route through `q.residentJob(id)` (returning `ErrJobNotResident` for non-resident jobs) or explicitly call `q.hydrateJobLocked(ctx, job)`. Public accessors (`Job.Manifest()`, `Job.Progress()`, and all methods on `*Manifest`) are thread-safe and nil-guarded. The checked-pointer residency convention (`residentJob` + safe accessors) is retained over a compile-time type-level separation (`ResidentJob` vs `QueuedJob`), avoiding struct duplication and allocation overhead on promotion/eviction cycles.
