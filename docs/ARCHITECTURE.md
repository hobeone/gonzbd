...
> **The saving is reachable again, and only for a file whose bytes tile it exactly.** The verdict below reads `FileProgress.AssembledCRC32`. It was unwritten for a period — `Queue.SetFileCRC32` had no production caller after the assembler lost its CRC authority (#349) — so every par2-tracked file reported `NoCRC` and the recovery volumes were always fetched. `Application.recordAssembledCRC` now supplies the value from the durability record: a file whose articles all abut collapses to a **single** `durable_runs` row at offset 0, and that row's `crc32` *is* the whole-file CRC, already combined, with no read of the file. A file that keeps more than one row — a hole, or an article that overlaps a sibling — supplies no CRC, still reports `NoCRC`, and still fetches every volume. That direction is safe: it costs bandwidth on a file whose bytes are in doubt and never ships an unrepaired one. See [`docs/durability-contract.md`](durability-contract.md) §4 and [`docs/post-processing-contract.md`](post-processing-contract.md) § *Verification bypass guarantees*.
