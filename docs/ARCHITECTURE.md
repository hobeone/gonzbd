...
7.  **Subsystem Start**: Invokes `application.Start()`, which first re-derives every **downloading** job's work set from what is actually on stable storage (`resumeAllJobs` — synchronous, and deliberately before the downloader can dispatch, so bytes already fsynced are never requested again), then boots the background goroutines for the pipeline, downloader, and post-processor. The sweep is authoritative: an article a previous run recorded as done but whose bytes are gone comes back Outstanding. Jobs past downloading are skipped, because in post-processing the assembler is no longer the only writer of their files and the download-time fact log no longer describes them.
