...
7.  **Assembly**: The `pipeline` hands decoded parts to the `assembler`, which writes them to their exact byte offset in the target file using `pwrite`. This allows for out-of-order assembly as segments arrive. The article's decoded CRC32 travels with it and is carried through to the barrier's drain report; nothing is persisted about the article at this point.
8.  **Durability barrier**: An article is *not* done because it was written. `durability.Barrier` runs a checkpoint per job — on a time bound (30s), a byte bound (64 MiB), at file completion, and at clean shutdown — which drains the write cache, `fsync`s every open file, commits the drained articles to `durable_runs` (one row per contiguous run of articles that were fsynced together), and only then mints a `DurableProof` that `Queue.AckDurable` requires. `DurableProof` has no exported constructor outside `internal/durability`, so no caller outside that package can name an article in a proof and the ordering is a compile-time guarantee on *that* path rather than a convention. It is a bound on the proof's payload, not a claim that nothing else in the queue can mark an article done — the resume-seeding paths deliberately can, on stable-storage evidence. See [`docs/durability-contract.md`](durability-contract.md) §1. A storage fault here stalls or fails the *job* and never marks an article failed. See [`docs/durability-contract.md`](durability-contract.md).
...
- **Manifest/Progress split**: a `Job` holds an immutable `Manifest` (parsed-once article/file structure — subjects, byte counts, the flat global article arrays) and a mutable `JobProgress` (per-article done/failed/emitted state, per-file assembly bookkeeping, job-level counters). `Manifest` is safe to share by reference across every `Snapshot`/`SnapshotJob` clone since nothing mutates it after construction; `JobProgress` is deep-copied per clone. External packages reach both only through `Job.Manifest()`/`Job.Progress()` accessor methods, and neither type exposes a mutating exported method. Those accessors take a per-`Job` `residencyMu` rather than reading the fields directly, so a reader never sees a half-swapped pair while lazy eviction and promotion reassign both fields. External packages no longer hold a live `*Job` at all — #463 deleted `Queue.Get` and `Queue.List`, the two exits for one, and `queue.TestNoExportedDoorHandsOutALiveJob` enumerates the exported surface from source to keep it that way. That deletion removed the contention rather than moving it: every remaining live-`Job` reader of the pair also holds `q.mu` (`cloneJob`'s callers take `RLock`; `setResidency` on a live job runs under `Lock`), so `residencyMu` is uncontended on every path enumerated today. Whether it can now be removed is tracked in #476. `residencyMu` covers *which* `Manifest`/`JobProgress` a job points to and nothing else — `Manifest` contents are immutable, but `JobProgress` fields are mutated in place under `q.mu` and remain unsynchronized for readers outside the package.
- **Article Addressing**: an article is addressed by its global index into `Manifest`'s flat arrays, never by message-ID. Every mutation entry point carries indices in the shape that suits it — `MarkArticleEmittedByIdx`/`ClearArticleEmittedByIdx` one `artIdx`, `AckPermanentFailure` a `[]int32`, `AckDurable` a `durability.DurableProof` whose `Articles()` payload holds them — and every caller already has one, so no by-ID lookup structure exists. Derived counters on `JobProgress` (`Pending`, `PendingArticles`) are excluded from JSON and recomputed on load via `JobProgress.recompute(m)`, which takes the manifest; the transient `Emitted` bit is likewise excluded so a restart re-dispatches articles no barrier had made durable before the crash (`Done` means a completed `fsync` covered the bytes — see `nntp-downloader-contract.md` §5 and [`docs/durability-contract.md`](durability-contract.md)).
...
> **The saving is reachable again, and only for a file whose bytes tile it exactly.** The verdict below reads `FileProgress.AssembledCRC32`. It was unwritten for a period — `Queue.SetFileCRC32FromRuns` had no production caller after the assembler lost its CRC authority (#349) — so every par2-tracked file reported `NoCRC` and the recovery volumes were always fetched. `Application.recordAssembledCRC` now supplies the value from the durability record: a file whose articles all abut collapses to a **single** `durable_runs` row at offset 0 covering every article of the file, and that row's `crc32` *is* the whole-file CRC, already combined, with no read of the file. A file that keeps more than one row — a hole, or an article that overlaps a sibling — supplies no CRC, and neither does one whose single row fails to account for every article, which is what a permanently failed article or a discarded same-offset duplicate leaves behind. All of them still report `NoCRC` and still fetch every volume. That direction is safe: it costs bandwidth on a file whose bytes are in doubt and never ships an unrepaired one. See [`docs/durability-contract.md`](durability-contract.md) §4 and [`docs/post-processing-contract.md`](post-processing-contract.md) § *Verification bypass guarantees*.

- **Fetch policy, not a boolean**: every file carries a `FetchPolicy` on `JobProgress`, persisted as `job_files.fetch_policy`. `FetchAlways` is the default; `FetchIfNeeded` is a recovery volume awaiting the verdict; `FetchNever` is one the verdict ruled unnecessary. A job's file set never changes after `NewJob` — a volume that is ruled out is *marked*, not removed, so every `file_index` stays valid for the job's whole life. See [`docs/queue-lifecycle.md`](queue-lifecycle.md).
...
- **Decision = identify, then verify**: at download-complete (`handleFileComplete`), `par2.Assess` reads the on-disk index once and answers three things together — which par2 entry each delivered file *is* (by `Hash16k` content, since an obfuscated name says nothing), whether that file is intact (its recorded CRC against par2's), and which relocations would follow. `par2Verdict` then reads that answer, in two steps rather than one. **Accounting first:** if some entries were identified and others were not, repair may be possible and the volumes are fetched; if *nothing* was identified, they are not (the Layout B case below). **Then verification:** for a fully accounted set, repair is needed iff `Mismatched + NoCRC + Unverified > 0`. A missing or unusable index skips both and falls back to fetching all volumes. A file the durability record cannot supply a whole-file CRC for reports `NoCRC` and forces the fetch — see the note above. The QuickCheck stage consumes the same `Assess` call, so the two cannot reach different verdicts about one directory.
- **The download path decides; it never renames**: `Assess` reports the moves that would put each file at its par2 path, and this path discards them. Relocation belongs to post-processing's `quickcheck` stage, ahead of the `repair` stage that needs the files there. Renaming here was tried and removed (#494): it existed only so that name-based verification would have corrected names to match against, and once identification became content-based it bought the verdict nothing — while costing a real defect, since `JobProgress.Filename` cannot hold a path, so a file relocated into a subdirectory could not be recorded truthfully and the startup resume sweep would re-download it in full.
- **Where relocation does happen, identification precedes it, and the type enforces that**: `Assess` reports renames without applying them, and `par2.ApplyRenames` takes the whole `Assessment`, so a verdict is always computed from pre-rename state. The reverse order was #492 and #494 — verification matched par2 entries by *name*, renaming invalidated exactly those names, and the ambiguous "nothing matched" result was then read as proof the set protected other files and used to **discard** the volumes on a healthy download. Content identification is what makes that signature meaningful: it now distinguishes a Layout B post, where nothing delivered matches any entry and the volumes provably cannot be spent (the sole repair pass runs *before* unpack), from a partial shortfall, which still fetches.
...
- **No ack path**: the assembler cannot mark an article done, record a whole-file CRC, or truncate. `FileWriter` reports only that bytes reached `WriteAt`; `durability.Barrier` reaches it through the worker's control channel to drain, fsync, stat and truncate, and is the only component that resolves an article *during a download*. The startup resume sweep also resolves articles, through `Queue.ReplaceFromRuns`, on stable-storage evidence rather than a barrier's fsync — see §Durability barrier above, which states the bound precisely. See [`docs/durability-contract.md`](durability-contract.md).
...
7.  **Subsystem Start**: Invokes `application.Start()`, which first re-derives every **downloading** job's work set from what is actually on stable storage (`resumeAllJobs` — synchronous, and deliberately before the downloader can dispatch, so bytes already fsynced are never requested again), then boots the background goroutines for the pipeline, downloader, and post-processor. The sweep does **one `stat` per file and no reads**: it trusts the durability record, and its whole mutation budget is to *delete* a file's rows when the file on disk is shorter than they claim. It is authoritative over the files it actually stats: for those, an article a previous run recorded as done but whose bytes are gone comes back Outstanding. A file it never reached — an unresolved filename, or one behind a storage fault — is left exactly as it was, because absence of a reading is not a finding of absence. Paused jobs are swept too — a paused job is mid-download, and it is where a storage stall leaves one — but jobs past downloading are skipped, because in post-processing the assembler is no longer the only writer of their files and the download-time record no longer describes them.
...
10. **Wait**: Blocks until a termination signal (SIGINT/SIGTERM) is received, then performs a graceful shutdown (stop producers → **checkpoint** → drain consumers → cancel context → wait → cleanup). The checkpoint step is the clean-shutdown durability barrier, and it has to sit exactly there: after the downloader stops so no new article can arrive, and before the assembler stops so the file handles it fsyncs still exist. Without it, everything downloaded since the last checkpoint is re-fetched on the next start. See `Application.stopWorkers` and [`docs/durability-contract.md`](durability-contract.md) § *The checkpoint cadence*.
