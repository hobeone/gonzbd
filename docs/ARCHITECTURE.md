...
- **Single Progress Writer**: `Checkpointer` (`internal/checkpoint`) is the only thing that *updates* `job_files` or *inserts* into `failed_articles` in SQLite, batching and coalescing from job snapshots. The SQL belongs to `durability.Store`, which owns both tables along with `durable_runs`; the checkpointer reaches it through `Store.SaveProgress`, and the adapter in `internal/app` that maps a `job.Checkpoint` to the store's types is that method's only production caller. The checkpointer is not the only writer of those tables: ingestion and retry seed the initial `job_files` rows through `Store.Admit`, and rows are deleted through the store's per-table discards from three functions in `internal/app` — `deleteJobDurability`, `dropJobDurability` and `RetryHistoryJob` (`git grep -nE '\.(DiscardFileRows|DiscardFailedArticles)\(' -- '*.go' ':!*_test.go'` returns 3 lines) — plus the history purge in `internal/history`, which drops retained durability rows when a history entry goes. What the checkpointer owns is progress, not row lifetime — `docs/durability-contract.md` § One record carries the deleter enumeration and the command behind it.
...
- **Clean verdict**: `DiscardDeferredPar2` moves every still-`FetchIfNeeded` volume to `FetchNever`, and only `outcomeClean` reaches it. No *verdict* promotes it back within a run — `Job.undeferRecovery` skips anything that is not `FetchIfNeeded`. Hydration is the exception, and it is not a verdict: `appResidency.Hydrate` restores the policy from `job_files`, so an eviction re-hydrates whatever the row holds. That is safe only while every writer of the policy marks the job for checkpointing, which is why both verdicts call `Application.markFetchPolicyDirty` — a flush writes only jobs already marked, so an unmarked verdict would be undone by the next eviction rather than merely delayed. A retry does not inherit the previous attempt's policy at all — it rebuilds the job from scratch through `BuildIngestJob`, whose own classification re-derives the policy from current config: `FetchIfNeeded` for a recovery volume while `downloads.on_demand_par2` is enabled, and `FetchAlways` once it is disabled. So a retry re-derives the verdict by re-verifying instead of re-downloading volumes the oracle already ruled out, and honours a setting the user has changed since the failed attempt (`TestRetryHistoryJob_ConfigurationIsHonoured`, #329).
- **Unidentifiable verdict is held, not discarded**: `outcomeUnknown` calls `SetPar2ReleaseReason` but neither `DiscardDeferredPar2` nor `UndeferRecoveryVolumes` — the volumes stay `FetchIfNeeded` ("held" in the UI, `internal/api/queue.go`'s `fileState`) rather than `FetchNever` ("skipped"), because a verdict that could not identify anything has not earned the "skipped" label. Holding does not rescue anything: nothing promotes a held volume after the job finalizes, and a retry rebuilds the job from scratch through `BuildIngestJob` rather than inheriting either policy, so a retry behaves the same under either policy for a stronger reason than before — neither one survives to be inherited. What it buys is that the on-disk state does not assert a verdict that was never taken.
...
- **SQLite Persistence**: Both active queue state (`dispatch_jobs`, `job_files`, `durable_runs`, `failed_articles`) and completed jobs (`history`, `history_job_files`) are stored in `history.db`. The schema is a single canonical `goose` migration (`001_initial.sql`) that descends from the original Python implementation's history table and has diverged from it. It is deliberately not interchangeable: `history.Open` refuses any database this build's migrations did not write.
...
6.  **Application Core**: Constructs the `app.Application` orchestrator, which initializes the internal `dispatcher`, `downloader`, `assembler`, and `postProcessor`.
