...
> **The saving is currently unreachable, and this section describes the mechanism rather than the observed behaviour.** The verdict below reads `FileProgress.AssembledCRC32`, and nothing writes it: `Queue.SetFileCRC32` has no production caller since the assembler lost its CRC authority (#349, and the durability rewrite). Every par2-tracked file therefore reports `NoCRC`, `par2NeedsRecovery` returns true unconditionally, and the **clean verdict path (`DiscardDeferredPar2` → `FetchNever`) is never taken** — the recovery volumes are always fetched. The direction is safe: it costs bandwidth on an intact download and never ships an unrepaired one. The honest replacement is `durability.FileExtent.PrefixCRC` guarded by `HasPrefixCRC`, which the resumer already derives and which nothing threads to `SetFileCRC32`. See [`docs/durability-contract.md`](durability-contract.md) § *Open gaps* and [`docs/post-processing-contract.md`](post-processing-contract.md) § *Verification bypass guarantees*.
...
- **Clean verdict** (not currently reachable — see the note above): `DiscardDeferredPar2` moves every still-`FetchIfNeeded` volume to `FetchNever`. That state is terminal within a run — `undeferRecoveryLocked` skips anything that is not `FetchIfNeeded`, so nothing promotes it back. Only `ResetForRetry` leaves it, downgrading to `FetchIfNeeded` rather than to `FetchAlways`, so a retry re-derives the verdict by re-verifying instead of re-downloading volumes the oracle already ruled out.
