...
>
> This plan's **CRC half** — stopping the barrier publishing a whole-file CRC it
> cannot justify — shipped in #410 and is current. **The `FileWriter`-side
> detection it describes was never built and should not be.** Two reviewers
> broke it five ways; the decisive one is that `req.Offset` is
...
>
> Detection was rebuilt from the opposite direction and shipped separately —
> **not** in #410, which contains none of it: the durability layer classifies
> why `verifiedPrefix` stopped walking a file's Class A facts, which carry
> length, persist across restarts, and are ordered by the query rather than by
> insertion. See `2026-08-20-overlap-detection-design.md`.
>
