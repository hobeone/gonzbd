...
- **De-duplication across checkpoints** is a quality measure, deliberately not a guard, because `SetWarning` is idempotent. If it proves noisy in practice the fix is in the reporting path, **not** persisted state on `FileExtent`.
...
- **A finding alongside an error is undefined and must be decided in Task 2.** `Run` can classify an overlap in phase 3 and then fail at `Commit` or `AckDurable`, and `checkpointJob`'s error branch returns early at three places. Either findings are returned on the nil-error path only, or they are reportable regardless — pick one and say which at the signature, rather than leaving it to whichever branch the implementer writes first.
