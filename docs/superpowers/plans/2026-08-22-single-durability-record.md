...
1. **Are byte offsets monotonic in article index for well-formed yEnc?** The run carries an index *range*, assuming offset-contiguity implies index-contiguity.
   *Probe:* check whether `=ypart begin=` is non-decreasing in segment number across a sample of real NZBs, and what `internal/nzb` guarantees about segment ordering.
   *Branches:* (a) it holds normally → runs collapse and the design pays off; (b) it commonly does not → runs rarely merge, storage degrades to one row per article, and the whole-file CRC rarely materialises. **(b) is not a stop** — the design still works, it just stops being an improvement, and that should be known before Task 2 rather than after Task 8.
