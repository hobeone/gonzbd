...
| `direct_unpack` | bool | false | Extract RAR volumes during download (requires enable_unrar) |
...
- **Forwarding headers (`X-Forwarded-For`, `Forwarded`, `X-Real-IP`)**: Consulted only after the direct peer already qualifies as trusted — a public client cannot spoof trust by appending a forged header, since its own peer IP fails the check first. When `general.verify_xff_header = true`, every hop in the single header named by `general.trusted_forward_header` (default `x-forwarded-for`) must also be trusted; the other two headers are not consulted at all, even if present — an explicit choice, not "whichever is present," because a proxy that reliably manages one header may pass another through from the client untouched, and auto-selecting by presence would let a client inject a forged value into the unmanaged header and have it silently win (mirrors nginx's `real_ip_header` directive). When `verify_xff_header` is `false` (the default), a request with **no** forwarding header trusts the peer alone (unchanged); a request where **any** of these three headers **is** present fails closed instead of trusting the peer, since the header's mere presence signals a proxy may be interposed and an unverified peer address (which the proxy controls) proves nothing about the real client — this closes the reverse-proxy-on-loopback gap where every proxied request otherwise arrives with `RemoteAddr = 127.0.0.1` (issue #94).
