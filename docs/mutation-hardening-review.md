...
| 22 | `6dac1cb` | test(config): harden config validation, parsing, and defaults test coverage | done | Test-only across 4 files. Most assertions verified correct against `validate.go`/`set.go`/`types.go` (port/percent/permissions/dirscan/HTTPS boundary checks, servers-slice JSON `Set`, schedule duplicate-name validation). **Issue found and fixed (production bug)**: `TestParseByteSize_BoundaryMaxInt64` asserted `ParseByteSize("9007199254740992K")` (= 2^63 bytes) returns `err == nil`, which passed but encoded a real bug — `float64(maxInt64)` itself rounds up to 2^63, so `bytes > float64(maxInt64)` was false at exactly 2^63, and `ByteSize(bytes)` then wrapped the float-to-int64 conversion to `math.MinInt64`, silently producing a large *negative* `ByteSize` for a config value that should be rejected as overflow. Fixed `types.go`'s comparison from `>` to `>=` (with explanatory comment) and rewrote the test to assert an overflow error at 2^63 and a clean positive parse at 2^63-1024 (one float64 ULP below). Proved red on the unfixed comparison (`got nil` failure), green after the fix. Verified `go test ./internal/config/... -race -count=1`, `go test ./...`, `golangci-lint run` 0 issues. |
...
- Tier 2 reviewed: 8 / 12
- Extra (post-scope): `4a6bc09` not yet reviewed — add to tier 2 queue

## Next up
Continue tier 2 with #24 `2804ad1` (scheduler — unit tests to eliminate lived mutants).
