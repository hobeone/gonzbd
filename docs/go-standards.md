...
- **Dispatcher internal locking**: `d.mu sync.Mutex` guards the per-job bookkeeping maps; `d.storeMu sync.Mutex` separately serializes store writes so a Save cannot race a Delete. Neither may be held across a call into `sched` or `Residency.Hydrate` — take it, touch one map, release.
...
- **All transient fields (`Pending`, `pendingArticles`, `emitted`) are excluded from the persisted shape.** They are recomputed by `JobProgress.recompute(m)` on load, and `emitted` is additionally cleared by `Job.ClearEmittedForReload` so a restart re-dispatches anything no barrier made durable. If you add new transient state, follow this pattern and ensure it is initialized on both paths that produce a `Job`: `Dispatcher.Add` for a newly ingested job, and `Dispatcher.restore` for one replayed from the store.

- **`Job.ClearEmittedForReload` is the self-healing reset.** It walks the manifest through `JobProgress.resetForReload` and ends in `JobProgress.recompute(m)`, rebuilding all counters from ground truth. If you suspect counter drift during development, reaching `recompute(m)` will correct it.
